...
from scanner.plugin import JavaASTPlugin
from scanner.plugin import ManifestPlugin
from scanner.plugin import PluginObserver
from scanner.plugin import get_plugin_source, get_plugins
...
class Scanner(object):
    ...
    def _run_checks(self, plugins):
        ...
        coroutine_plugins = list(coroutine_plugin for coroutine_plugin in plugins if isinstance(coroutine_plugin, CoroutinePlugin))
        ...
        # Finalize plugins if they have a finalize method
        for plugin in plugins:
            if hasattr(plugin, 'finalize') and callable(getattr(plugin, 'finalize')):
                plugin.finalize()

        # Extend issues after finalization
        for plugin in plugins:
            self.issues.extend(plugin.issues)
